...
house_subsidy %>%
  ...
  scale_fill_manual(values = palette2) +
  labs(x="Take Credit", y="Count",
       title = "Time associated with contacted association with the likelihood of taking credit"))
...
cost_benefit_table <-
  ...
  mutate(HCD_Expenditure =
           case_when(Variable == "True_Negative" ~ Count*0,
                Variable == "True_Positive" ~ (Count*2850)+(Count*5000),
  ...
  while (x <= 1) {
    ...
    this_prediction <-
      ...
      summarize(True_Negative = sum(n[predOutcome==0 & Outcome==0]),
                ...
                False_Positive = sum(n[predOutcome==1 & Outcome==0])) %>%
      gather(Variable, Count) %>%
      mutate(ValueAdded_Minus_Expen =
               if_else(Variable == "True_Negative", Count * 0,
               if_else(Variable == "True_Positive", (Count*1000)+(Count*56000)-(Count*2850)-(Count*5000),
               if_else(Variable == "False_Negative", Count*0,
               if_else(Variable == "False_Positive", Count*2850,0)))), Threshold = x) %>%
      mutate(Number_Credits=
               case_when(Variable == "True_Negative" ~ Count*0,
...
ggplot(whichThreshold_revenue)+
  geom_line(aes(x = Threshold, y = ValueAdded_Minus_Expen))+
  geom_vline(xintercept =  pull(arrange(whichThreshold_revenue, -ValueAdded_Minus_Expen)[1,1]))+
