...
# Compare Models
ERROR HERE with kitchen sink model
...
#kitchensink1 <- glm(y_numeric ~ .,
                   # data=housingTrain %>% 
                     # dplyr::select(age, job, marital, education, taxLien, mortgage,taxbill_in_phl,contact,month,day_of_week,campaign,pdays,previous,
                                  #  poutcome, unemploy_rate, cons.price.idx,
                                 #   cons.conf.idx, inflation_rate, spent_on_repairs),
                   # family="binomial" (link="logit"))

#kitchensink2 <- glm(y_numeric ~ .,
                    #data=housingTrain %>% 
                     # dplyr::select(-X, -y, -y_numeric, -Season, -Education_group,-Age_group,-Employment),
                    #family="binomial" (link="logit"))

#kitchensink3 <- glm(y_numeric ~ .,
                   # data=housingTrain %>% 
...
ggplot(whichThreshold_credits)+
  ...
  geom_vline(xintercept =  pull(arrange(whichThreshold_credits, -Revenue)[1,1]))+
